...
import { ArrowLeft, CheckCircle2, Clock3, Mail, MessageSquare, User } from 'lucide-react';
    ...
    if (!feedback) {
        return (
            <div className="space-y-6">
                <Link to="/admin/feedback" className="inline-flex items-center gap-2 text-primary font-bold hover:underline">
                ...
                <div className="rounded-2xl border border-surface-container-high bg-surface p-6 text-on-surface-variant">
                    Không tìm thấy khiếu nại với mã <span className="font-bold text-on-surface">{decodedId}</span>.
                </div>
            </div>
        );
    }
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <header className="space-y-3">
                ...
                <h1 className="text-3xl font-serif italic text-on-surface">
                    Chi tiết <span className="not-italic text-primary font-black">khiếu nại</span>
                </h1>
                <p className="text-on-surface-variant font-medium">Mã đơn: {feedback.id}</p>
            </header>

            <section className="bg-surface-container-lowest rounded-2xl border border-surface-container-highest p-6 space-y-5">
                <div>
                    <p className="text-xs font-black uppercase tracking-widest text-primary mb-2">Tiêu đề</p>
                    <h2 className="text-2xl font-bold text-on-surface">{feedback.subject}</h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-on-surface-variant">
                    ...
                    <p className="inline-flex items-center gap-2">
                        ...
                        Trạng thái hiện tại: <span className="font-bold text-on-surface">{statusText(feedback.status)}</span>
                    </p>
                </div>

                <div className="rounded-2xl border border-surface-container-high bg-surface p-5">
                    <p className="text-sm leading-relaxed text-on-surface">{feedback.message}</p>
                </div>
            </section>
            ...
            <section className="bg-surface-container-lowest rounded-2xl border border-surface-container-highest p-6 space-y-4">
                ...
                <div className="space-y-2">
                    <label className="text-sm font-bold text-on-surface">Cập nhật trạng thái</label>
                    <select
                        value={status}
                ...
                <div className="space-y-2">
                    <label className="text-sm font-bold text-on-surface">Ghi chú phản hồi admin</label>
                    <textarea
                        rows={4}
                        value={note}
